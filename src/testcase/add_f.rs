use crate::common;
use crate::test::{
    make_random, RandomTest, TestCase, TestCommon, TestPtx,
};
use rand::Rng;
...
impl Default for AddF32 {
    fn default() -> Self {
        Self {
            rnd: common::Rounding::Rn,
            ftz: false,
            sat: false,
...
impl TestCommon for AddF32 {
    ...
    fn host_verify(&self, input: Self::Input, output: Self::Output) -> Result<(), Self::Output> {
        ...
        if expected.to_bits() == output.to_bits() {
            ...
            Ok(())
        } else if expected == 0.0 && output == 0.0 {             // Allow +0.0 and -0.0 to be treated as equivalent.
            Ok(())
        } else {
            Err(expected)
        }
...
impl RandomTest for AddF32 {
    fn generate<R: Rng + Sized>(&self, rng: &mut R) -> Self::Input {
        let special_cases: [(f32, f32); 23] = [
                ...
                (f32::from_bits(rng.gen::<u32>()), crate::common::MAX_POSITIVE_SUBNORMAL),
                (f32::from_bits(rng.gen::<u32>()), crate::common::MAX_NEGATIVE_SUBNORMAL),
                ...
                (f32::from_bits(rng.gen::<u32>()), crate::common::MIN_POSITIVE_SUBNORMAL),
                (f32::from_bits(rng.gen::<u32>()), crate::common::MIN_NEGATIVE_SUBNORMAL),
        ...
        if rng.gen_bool(0.01) {
            ...
            (f32::from_bits(rng.gen::<u32>()), f32::from_bits(rng.gen::<u32>()))
        }
...
fn add_host_f32(a: f32, b: f32, rnd: common::Rounding, ftz: bool, sat: bool) -> f32 {
    ...
    let mut result = rnd.with(|| exact as f32);
    ...
    for &rounding in &roundings {
        for &ftz in &ftz_options {
            for &sat in &sat_options {
                let name = format!("add_f32_{}_{}_{}", rounding.as_str(), if ftz {"ftz"} else {"noftz"}, if sat {"sat"} else {"nosat"});
                tests.push(TestCase::new(name, make_random(AddF32::new(rounding, ftz, sat))));
            }
        }
    }
    tests
